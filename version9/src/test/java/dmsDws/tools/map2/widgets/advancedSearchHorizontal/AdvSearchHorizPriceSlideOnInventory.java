 ...
 */
public class AdvSearchHorizPriceSlideOnInventory extends AdvSearchHorizTestBaseInventory {

    private static final Logger LOG = LogFactory.getLogger(AdvSearchHorizPriceSlideOnInventory.class);
    ...
    @Test(description = "Price on inventory page - price from should taken from URL")
    public void priceFromIsTakenFromUrl() {
        driver.get(PropertyLoader.loadProperty(PropertySource.URL,"dws.url") + PropertyLoader.loadProperty(PropertySource.URL,"inventory.url"));
        waitForJSandJQueryToLoad();
        String priceFromChanged = manager.getSearchDws(driver).getPriceFromChanged(3);
        LOG.info("priceFromChanged is " + priceFromChanged);
        String priceToChanged = manager.getSearchDws(driver).getPriceToChanged(3);
        LOG.info("priceToChanged is " + priceToChanged);
        driver.get(PropertyLoader.loadProperty(PropertySource.URL,"dws.url") + PropertyLoader.loadProperty(PropertySource.URL,"inventory.url") + "_pricefrom_" + priceFromChanged + "_priceto_" + priceToChanged);
        waitForJSandJQueryToLoad();
        Assert.assertEquals(manager.getSearchDws(driver).getPriceSlideFromValueWithoutSpace(), priceFromChanged);
    }

    @Test(description = "Price on inventory page - price to should taken from URL")
    public void priceToIsTakenFromUrl() {
        driver.get(PropertyLoader.loadProperty(PropertySource.URL,"dws.url") + PropertyLoader.loadProperty(PropertySource.URL,"inventory.url"));
        waitForJSandJQueryToLoad();
        String priceFromChanged = manager.getSearchDws(driver).getPriceFromChanged(3);
        LOG.info("priceFromChanged is " + priceFromChanged);
        ...
        driver.get(PropertyLoader.loadProperty(PropertySource.URL,"dws.url") + PropertyLoader.loadProperty(PropertySource.URL,"inventory.url") + "_pricefrom_" + priceFromChanged + "_priceto_" + priceToChanged);
        waitForJSandJQueryToLoad();
        Assert.assertEquals(manager.getSearchDws(driver).getPriceSlideToValueWithoutSpace(), priceToChanged);
    }

    @Test(description = "Price on inventory page - price from = price to - should taken from URL")
    public void equalPricesFromAreTakenFromURL() {
        driver.get(PropertyLoader.loadProperty(PropertySource.URL,"dws.url") + PropertyLoader.loadProperty(PropertySource.URL,"inventory.url"));
        waitForJSandJQueryToLoad();
        String priceFromChanged = manager.getSearchDws(driver).getPriceFromChanged(2);
        LOG.info("priceFromChanged is " + priceFromChanged);
        driver.get(PropertyLoader.loadProperty(PropertySource.URL,"dws.url") + PropertyLoader.loadProperty(PropertySource.URL,"inventory.url") + "_pricefrom_" + priceFromChanged + "_priceto_" + priceFromChanged);
        waitForJSandJQueryToLoad();
        Assert.assertEquals(manager.getSearchDws(driver).getPriceFromValueVisibility(), "visible");
    }
}
